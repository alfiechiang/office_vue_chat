<template>
  <div class="container" id="app">
    <div class="row">
      <div class="col-md-12">
        <div class="page-header">
          <h2 class="text-center">欢迎来到《Go 语言编程之旅：一起用 Go 做项目》聊天室</h2>
        </div>
      </div>
    </div>
    <div class="row">
      <div class="col-md-1"></div>
      <div class="col-md-6">
        <div>聊天内容</div>
        <div class="msg-list" id="msg-list">
          <div class="message" v-for="msg in msglist" v-bind:key=" msg.content" v-bind:class="{ system: msg.type>0, myself: msg.user.nickname==curUser.nickname }">
            <div class="meta" v-if="msg.type==0"><span class="author">{{ msg.user.nickname }}</span> at {{formatDate(msg.msg_time)}} {{calc(msg)}}</div>
            <div>
              <span class="content" style="white-space: pre-wrap;">{{msg.content}}</span>
            </div>
          </div>
        </div>
      </div>
      <div class="col-md-4">
        <div>当前在线用户数：<font color="red">{{onlineUserNum}}</font>
        </div>
        <div class="user-list">
          <div class="user" v-for="user in users" v-bind:key=" user.nickname">
            用户：@{{user.nickname }} 加入时间：{{formatDate(user.enter_at) }}
          </div>
        </div>
      </div>
    </div>
    <div class="row">
      <div class="col-md-1"></div>
      <div class="col-md-10">
        <div class="user-input">
          <div class="usertip text-center">{{ usertip }}</div>
          <div class="form-inline has-success text-center" style="margin-bottom: 10px;">
            <div class="input-group">
              <span class="input-group-addon">您的昵称</span>
              <input type="text" v-model="curUser.nickname" v-bind:disabled="joined" class="form-control" aria-describedby="inputGroupSuccess1Status">
            </div>
            <input type="submit" class="form-control btn-primary text-center" v-on:click="leavechat" v-if="joined" value="离开聊天室">
            <input type="submit" class="form-control btn-primary text-center" @click="joinchat" value="进入聊天室">
          </div>
          <textarea id="chat-content" rows="3" class="form-control" v-model="content" @keydown.enter.prevent.exact="sendChatContent" @keydown.meta.enter="lineFeed" @keydown.ctrl.enter="lineFeed" placeholder="在此收入聊天内容。ctrl/command+enter 换行，enter 发送"></textarea>&nbsp;
          <input type="button" value="发送(Enter)" class="btn-primary form-control" @click="sendChatContent">
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import Vue from 'vue';
// @ is an alias to /src

export default {
  name: "Chat",
  data() {
    return {
      msglist: [],
      content: "",
      curUser: {
        uid: 0,
        nickname: "",
        token: "",
      },
      usertip: "当前还未进入聊天室，请在下方「填上您的昵称」",

      // 是否已经加入聊天室
      joined: false,

      users: [],
      indexMap: {},
      gWS: "",
    };
  },
  mounted() {
    console.log("eeee");
  },
  computed: {
    onlineUserNum() {
      return this.users.length;
    },
  },
  methods: {
    joinchat() {
      if (this.curUser.nickname == "") {
        this.usertip = "昵称不能为空";
        return;
      }

      this.usertip = "";
      this.joined = true;

      if ("WebSocket" in window) {
        let host = location.hostname+":2021";
        // 打开一个 websocket 连接
        this.gWS = new WebSocket("ws://"+host+"/ws?nickname="+this.curUser.nickname+"&token="+this.curUser.token);
       //  this.gWS = new WebSocket("ws://localhost:2021/ws");

        this.gWS.onopen = function () {
          // WebSocket 已连接上的回调
        };
        this.gWS.onmessage = (evt)=>{
          let data = JSON.parse(evt.data);
          if (data.type == 4) {
            this.usertip = data.content;
            this.joined = false;
            return;
          } else if (data.type == 1) {
            // 欢迎消息
            this.curUser = data.user;
            localStorage.setItem("user", JSON.stringify(data.user));

            data.user = { nickname: "", uid: 0 };

            this.fetchUserList();
          } else if (data.type == 2) {
            // 某个用户进入
            let user = data.user;
            let len = this.users.length;
            this.users.push(user);
            this.indexMap[user.nickname] = len;
          } else if (data.type == 3) {
            // 某个用户退出
            let nickname = data.user.nickname;
            let idx = this.indexMap[nickname];

            this.users.splice(idx, 1);

            for (let i = idx; i < this.users.length; i++) {
              let nickname = this.users[i].nickname;
              this.indexMap[nickname] = i;
            }
          }

          this.addMsg2List(data);
        };

        this.gWS.onerror = function (evt) {
          console.log("发生错误：");
          console.log(evt);
        };

        this.gWS.onclose = function () {
          console.log("连接已关闭");
        };
      } else {
        alert("您的浏览器不支持 WebSocket!");
      }
    },
    leavechat() {
      this.gWS.close();

      this.msglist.splice(0);

      this.addMsg2List({
        user: { nickname: "" },
        type: 1,
        content: "您已离开聊天室，再见！",
      });

      this.users.splice(0);

      this.joined = false;
    },
    sendChatContent() {
      let msg = JSON.stringify({ content: this.content });
      this.gWS.send(msg);

      let data = {
        user: {
          nickname: this.curUser.nickname,
          uid: this.curUser.uid,
        },
        type: 0,
        content: this.content,
        msg_time: new Date().getTime(),
      };

      this.addMsg2List(data);
      this.content = "";
    },
    fetchUserList() {
      // XMLHttpRequest对象用于在后台与服务器交换数据
      var xhr = new XMLHttpRequest();
      //每当readyState改变时就会触发onreadystatechange函数
      //0: 请求未初始化
      //1: 服务器连接已建立
      //2: 请求已接收
      //3: 请求处理中
      //4: 请求已完成，且响应已就绪
      let url ="http://localhost:2021/user_list"
      xhr.open("GET", url, true);
      xhr.onreadystatechange = () =>{
        //readyStatus == 4说明请求已经完成
        if (xhr.readyState == 4 && xhr.status == 200) {
          //this.users.splice(0);
          let users = JSON.parse(xhr.responseText);
          for (let i in users) {
            let user = users[i];
            this.indexMap[user.nickname] = this.users.length;
            this.users.push(user);
          }
        }
      };
      //发送数据
      xhr.send();
    },
    lineFeed() {
      this.content = this.content + "\n";
    },
    formatDate(dateStr) {
      let d = new Date(dateStr);
      return d.toLocaleString();
    },
    calc(msg) {
      if (typeof msg.client_send_time == "undefined") {
        return "";
      }

      let send = new Date(msg.client_send_time);
      if (send.getFullYear() == 1) {
        return "";
      }

      let elaspe = msg.receive_time.getTime() - send.getTime();
      return "耗时 " + elaspe + "ms";
    },

    addMsg2List(data) {
      if (data.content == "") {
        return;
      }

      if (data.ats != null) {
        data.ats.forEach(function (nickname) {
          if (nickname == "@" + this.nickname) {
            this.usertip = "有人 @ 你了";
          }
        });
      }

      data.receive_time = new Date();

      if (this.msglist.length > 80) {
        this.msglist.splice(0, 40);
      }

      this.msglist.push(data);

      Vue.nextTick(function () {
        let msgList = document.querySelector("#msg-list");
        msgList.scrollTop = msgList.scrollHeight;
      });

      setTimeout(function () {
        this.usertip = "";
      }, 5000);
    },

    keepAlive() {
      // 表明异常退出了
      if (this.gWS.readyState == WebSocket.CLOSED && this.joined) {
        console.log("reconnect");
        this.joinchat();
      }
    },
  },
  delimiters: ["${", "}"],
};
</script>

 <style>
.msg-list {
  height: 400px;
  overflow: scroll;
  border: 1px solid #ccc;
  background-color: #f3f3f3;
  display: flex;
  flex-direction: column;
}
.message {
  margin: 15px 5px 5px 5px;
  padding: 5px;
  background-color: #fff;
}
.message {
  align-self: flex-start;
}
.message .meta {
  color: #ccc;
  font-size: 12px;
}
.message .author {
  color: #999;
  font-weight: bold;
}
.myself {
  background-color: #b0e46e !important;
  align-self: flex-end;
}
.myself .meta {
  color: #2b2b2b;
}

.system {
  background-color: #f3f3f3;
  color: #ccc;
  align-self: center;
}

.user-list {
  padding-left: 10px;
  height: 400px;
  overflow: scroll;
  border: 1px solid #ccc;
  background-color: #f3f3f3;
}
.user-list .user {
  background-color: #fff;
  margin: 5px;
}

.user-input {
  margin: 10px;
}
.usertip {
  color: red;
}
</style>
